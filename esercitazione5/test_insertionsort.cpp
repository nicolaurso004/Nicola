...
int main() {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        if (!is_sorted(v)) {
            std::cerr << "ERRORE: Insertion Sort ha fallito con i numeri!\n";
    ...
    if (!is_sorted(vs)) {
        std::cerr << "ERRORE: Insertion Sort ha fallito con le stringhe!\n";
        return EXIT_FAILURE;
    }

    std::cout << "Test Insertion Sort: SUPERATO CON SUCCESSO!\n";
    return EXIT_SUCCESS;
}